...
extendINTGr {k} [] N→W n w' ∉PF IPF = tt       
extendINTGr {k} ((n1 , p+ , n2) ∷ G) N→W n w' ∉PF IPF =  
                         ...
                         (sym (extNEqN→W {k}{N→W}{w'} n2 n 
                           (node∉DomNeq n ((n1 , p+ , n2) ∷ G) ∉PF n2 
                              (inj2 (inApp2 {L1 = Data.List.map proj1 G}
                                   ...
                                   (proj1 IPF) ) , 
   extendINTGr {k} G N→W n w' (λ H1 → ∉PF (inj2 (inApp2 {L1 = Data.List.map proj₁ G} 
...
extendINTΓ {k} G [] N→W n w' ∉PF IPF = tt
extendINTΓ {k} G ((p+ , A , n) ∷ Γ) N→W n' w' ∉PF IPF with compare n' n
extendINTΓ {k} G ((p+ , A , .m) ∷ Γ) N→W .m w' ∉PF IPF | equal m  =  
  ⊥-elim (∉PF (inj1 refl))
extendINTΓ {k} G (( p+ , A , .(suc (m1 + m2))) ∷ Γ) N→W .m1 w' ∉PF IPF | less m1 m2 = 
  (proj1 IPF , -- The use of IPF here is the reason I had to case split over the polarity.
   extendINTΓ G Γ N→W m1 w' (λ x → ∉PF (inj2 x)) (proj2 IPF))
extendINTΓ {k} G (( p+ , A , .m1) ∷ Γ) N→W .(suc (m1 + m2)) w' ∉PF IPF | greater m1 m2 = 
  (proj1 IPF) , (extendINTΓ G Γ N→W (suc (m1 + m2)) w' (λ x → ∉PF (inj2 x)) (proj2 IPF))
...
soundness-prop G Γ p+ n A k N→W = G⟦ G ⟧ k , N→W → Γ⟦ Γ ⟧ k , N→W → k ⟦ A ⟧ (N→W n)

soundness-prop G Γ p- n A k N→W = G⟦ G ⟧ k , N→W → Γ⟦ Γ ⟧ k , N→W → (k ⟦ A ⟧ (N→W n) → False)
  ...
  λ GPF ΓPF w' HR AINT → subst {A = W k} 
                                           ...
                                           (soundCF H k (extendN→W {k} N→W (suc (m1 + m2)) w')                                             
                                             (subst2 {W k} {W k} (λ w1 w2 → R k w1 w2) {N→W m1}
                                                ...
                                                (sym (extEq (suc (m1 + m2)))) 
                                                HR , 
                                                extendINTGr {k} G N→W (suc (m1 + m2)) w' n'InG GPF) 
                                             (subst {A = W k} (λ w → k ⟦ A ⟧ w) {w'}
                                                {extendN→W {k} N→W (suc (m1 + m2)) w' (suc (m1 + m2))} 
                                                (sym (extEq (suc (m1 + m2)))) 
                                                AINT , 
                                              extendINTΓ {k} G Γ N→W (suc (m1 + m2)) w' n'∉Γ ΓPF))
  ...
  λ GPF ΓPF w' HR AINT → subst {A = W k} 
                                           ...
                                           (soundCF H k (extendN→W {k} N→W m1 w')                                             
                                             (subst2 {W k} {W k} (λ w1 w2 → R k w1 w2) {N→W (suc (m1 + m2))}
                                               ...
                                               (sym (extEq m1)) 
                                               HR , 
                                               extendINTGr {k} G N→W m1 w' n'InG GPF)
                                             (subst {A = W k} (λ w → k ⟦ A ⟧ w) {w'}
                                               {extendN→W {k} N→W m1 w' m1} 
                                               (sym (extEq m1)) 
                                               AINT , 
                                               extendINTΓ {k} G Γ N→W m1 w' n'∉Γ ΓPF))

soundCF {G}{Γ}{p-}{n} (ifrImp {n' = n'}{A}{B} n'InG n'∉Γ H) k N→W with compare n n'    
soundCF {G}{Γ}{p-}{.n} (ifrImp {n' = .n}{A}{B} n'InG n'∉Γ H) k N→W | equal n = λ GPF ΓPF ∃PF → 
  soundCF H k (extendN→W {k} N→W n (proj1 ∃PF))             
            (mrefl k , extendINTGr G N→W n (proj1 ∃PF) n'InG GPF) 
            ((λ H' → proj1 (proj2 (proj2 ∃PF)) 
               (subst {A = W k} 
                      (λ w → k ⟦ A ⟧ w) 
                      {extendN→W {k} N→W n (proj1 ∃PF) n}
                      {proj1 ∃PF} (extEq n) H')) , 
            extendINTΓ {k} G Γ N→W n (proj1 ∃PF) n'∉Γ ΓPF) 
  ...
  λ GPF ΓPF ∃PF → 
    soundCF H k (extendN→W {k} N→W (suc (m1 + m2)) (proj1 ∃PF))              
              (subst2 
                 ...
                 (proj1 (proj2 ∃PF)) , 
               extendINTGr {k} G N→W (suc (m1 + m2)) (proj1 ∃PF) n'InG GPF)
              ((λ H' → 
                (proj1 (proj2 (proj2 ∃PF))) (subst 
                                                ...
                                                (extEq (suc (m1 + m2))) 
                                                H')) , 
               (extendINTΓ {k} G Γ N→W (suc (m1 + m2)) (proj1 ∃PF) n'∉Γ ΓPF))
              (subst {A = W k} 
                     (λ w → k ⟦ B ⟧ w) 
                     {proj1 ∃PF}
  ...
  λ GPF ΓPF ∃PF →  
    soundCF H k (extendN→W {k} N→W m1 (proj1 ∃PF))              
              (subst2 
                 ...
                 (proj1 (proj2 ∃PF)) , 
               extendINTGr {k} G N→W m1 (proj1 ∃PF) n'InG GPF)
              ((λ H' → 
                (proj1 (proj2 (proj2 ∃PF))) (subst 
                                                {A = W k} 
                                                (λ w → k ⟦ A ⟧ w) 
                                                ...
                                                (extEq m1) 
                                                H')) , 
               (extendINTΓ {k} G Γ N→W m1 (proj1 ∃PF) n'∉Γ ΓPF))
              (subst {A = W k} 
                     (λ w → k ⟦ B ⟧ w) 
                     {proj1 ∃PF}
                     {extendN→W {k} N→W m1 (proj1 ∃PF) m1}
...
soundCF {G}{Γ}{p+}{n} (ifrImpBar {n' = n'}{A}{B} HR Hinfr1 Hinfr2) k N→W = 
   λ GPF ΓPF → (N→W n') , 
                           (R→R GPF HR) , 
                           (soundCF {G}{Γ}{p-}{n'} Hinfr1 k N→W GPF ΓPF) , 
                           (soundCF {G}{Γ}{p+}{n'} Hinfr2 k N→W GPF ΓPF)

soundCF {G}{Γ}{p-}{n} (ifrImpBar {n' = n'}{A}{B} HR Hinfr1 Hinfr2) k N→W = 
   λ GPF ΓPF H → soundCF {G} {Γ} {p-} {n'} Hinfr2 k 
                                   N→W GPF ΓPF 
                                   (H (N→W n') 
                                      (R→R GPF HR) 
                                      (soundCF {G}{Γ}{p+}{n'} Hinfr1 k N→W GPF ΓPF))
...
soundCF {G}{Γ}{p+}{n} ifrUnit k N→W = λ GPF ΓPF → tt
soundCF {G}{Γ}{p-}{n} ifrUnit k N→W = λ GPF ΓPF H → H

soundCF {G}{Γ}{p+}{n} (ifrAnd H1 H2) k N→W = 
  λ GPF ΓPF →  (soundCF H1 k N→W GPF ΓPF , soundCF H2 k N→W GPF ΓPF) 

soundCF {G}{Γ}{p-}{n} (ifrAnd {A = A}{B = B} H1 H2) k N→W =
  λ GPF ΓPF H → [ (λ AINT → (soundCF H1 k N→W GPF ΓPF) AINT) , (λ BINT → (soundCF H2 k N→W GPF ΓPF) BINT) ]′ H

soundCF {G}{Γ}{p+}{n} (ifrAndBarL {A = A}{B} H) k N→W with (lem {k}{B}{N→W n}) 
... | yes H' = λ GPF ΓPF →  inj2 H' 
... |  no H' = λ GPF ΓPF →  inj1 (soundCF H k N→W GPF (H' , ΓPF))

soundCF {G}{Γ}{p+}{n} (ifrAndBarR {A = A}{B} H) k N→W with (lem {k}{A}{N→W n})
... | yes H' = λ GPF ΓPF →  inj1 H' 
... |  no H' = λ GPF ΓPF →  inj2 (soundCF H k N→W GPF (H' , ΓPF)) 

soundCF {G}{Γ}{p-}{n} (ifrAndBarL H) k N→W = 
  λ GPF ΓPF H' → (soundCF H k N→W GPF (proj2 H' , ΓPF)) (proj1 H') 

soundCF {G}{Γ}{p-}{n} (ifrAndBarR H) k N→W = 
  λ GPF ΓPF → λ H' → (soundCF H k N→W GPF (proj1 H' , ΓPF)) (proj2 H')  

soundCF {G}{Γ}{p+}{n} (ifrAxCut {n' = n'}{A}{B} inΓ H) k N→W with (lem {k}{A}{N→W n})
... | yes H' = λ GPF ΓPF → H' 
... | no H'  = λ GPF ΓPF → 
    ⊥-elim (soundCF H k N→W GPF (H' , ΓPF) 
...
soundCF {G}{Γ}{p-}{n} (ifrAxCut {n' = n'}{A}{B} inΓ H) k N→W = λ GPF ΓPF H' →
  (let split = inSplit Γ inΓ in 
     let Γ1 = proj1 split in 
       let Γ2 = proj1 (proj2 split) in
         proj1 (proj2 (split⟦Γ⟧ {k}{N→W}{Γ1}{(p- , B , n') ∷ Γ2} 
...
... | yes H' = λ GPF ΓPF → H'
... |  no H' = λ GPF ΓPF → ⊥-elim ((sound H2 k N→W GPF (H' , ΓPF))
                                         (sound H1 k N→W GPF (H' , ΓPF)))

sound {G}{Γ}{p-}{n} (ifrCut {n' = n'}{A}{B} H1 H2) k N→W = 
  λ GPF ΓPF H' → sound H2 k N→W GPF (H' , ΓPF)
                        (sound H1 k N→W GPF (H' , ΓPF))
