...
interp_mono {k} {w} {w'} r (And p+ f1 f2) (p1 , p2) = 
  (interp_mono {k} {w} {w'} r f1 p1) ,′ (interp_mono {k} {w} {w'} r f2 p2)
interp_mono {k} {w} {w'} r (And p- f1 f2) (inj₁ p1) = 
  inj₁ (interp_mono {k} {w} {w'} r f1 p1)
interp_mono {k} {w} {w'} r (And p- f1 f2) (inj₂ p1) = 
  inj₂ (interp_mono {k} {w} {w'} r f2 p1)
interp_mono {k} {w} {w'} r (Implies p+ f1 f2) p = 
  λ w'' r' p' → p w'' (mtrans k r r') p'
interp_mono {k} {w} {w'} r (Implies p- f1 f2) ( w'' , ( r' , p1 , p2 ) ) = 
  record { proj₁ = w'' ; proj₂ = ( mtrans k r' r , p1 , p2 ) }
...
expand⟦G⟧ : ∀{k : model}{N→W : Node → W k}{G G1 G2 : Graph}{e : Node × polarity × Node} 
  → G⟦ G ⟧ k , N→W  
  → e ∈ G 
  → G ≡ G1 ++ (e ∷ G2) 
  → G⟦ G1 ++ (e ∷ G2) ⟧ k , N→W 
...
split⟦G⟧ : ∀{k : model}{N→W : Node → W k}{G1 G2 : Graph} 
  → G⟦ G1 ++ G2 ⟧ k , N→W  
  → G⟦ G1 ⟧ k , N→W  × G⟦ G2 ⟧ k , N→W 
split⟦G⟧ {k}{N→W}{[]}{G2} GPF = Data.Unit.tt , GPF
split⟦G⟧ {k}{N→W}{(n1 , p+ , n2) ∷ G1}{G2} GPF = 
  (((proj1 GPF , proj1 (split⟦G⟧ {k}{N→W}{G1}{G2} (proj2 GPF)))) , 
   proj2 (split⟦G⟧ {k}{N→W}{G1}{G2} (proj2 GPF)))
split⟦G⟧ {k}{N→W}{(n1 , p- , n2) ∷ G1}{G2} GPF = 
  ((proj1 GPF , proj1 (split⟦G⟧ {k}{N→W}{G1}{G2} (proj2 GPF))) , 
   proj2 (split⟦G⟧ {k}{N→W}{G1}{G2} (proj2 GPF)))
...
R→R : ∀{k : model}{N→W : Node → W k}{G : Graph}{n1 n2 : Node}{p : polarity} 
  → G⟦ G ⟧ k , N→W 
  → G ⊢ (n1 , p , n2) 
  → R-prop k (N→W n1) (N→W n2) p
R→R {k}{N→W}{p = p+} _ RelRefl = mrefl k
R→R {k}{N→W}{p = p-} _ RelRefl = mrefl k
R→R {k}{N→W}{G}{n1}{n2}{p+} GPF (RelAx inPF) = 
  proj1 (proj2 (split⟦G⟧ {k}{N→W}{proj1 (inSplit G inPF)}{(n1 , p+ , n2) ∷ proj1 (proj2 (inSplit G inPF))} 
  (expand⟦G⟧ {k}{N→W}{G}{proj1 (inSplit G inPF)}{proj1 (proj2 (inSplit G inPF))}
            {(n1 , p+ , n2)} GPF inPF (proj2 (proj2 (inSplit G inPF))))))

R→R {k}{N→W}{G}{n1}{n2}{p-} GPF (RelAx inPF) = 
  proj1 (proj2 (split⟦G⟧ {k}{N→W}{proj1 (inSplit G inPF)}
  {(n1 , p- , n2) ∷ proj1 (proj2 (inSplit G inPF))} 
  (expand⟦G⟧ {k}{N→W}{G}{proj1 (inSplit G inPF)}{proj1 (proj2 (inSplit G inPF))}
            {(n1 , p- , n2)} GPF inPF (proj2 (proj2 (inSplit G inPF))))))
...
expand⟦Γ⟧ : ∀{Γ Γ1 Γ2 : Context}{k : model}{N→W : Node → W k}{e : polarity × form × Node} 
  → Γ⟦ Γ ⟧ k , N→W 
  → e ∈ Γ → Γ ≡ Γ1 ++ (e ∷ Γ2) 
  → Γ⟦ Γ1 ++ (e ∷ Γ2) ⟧ k , N→W 
expand⟦Γ⟧ {Γ}{k = k}{N→W} Γpf inPF eqPf = subst (λ Γ' → Γ⟦ Γ' ⟧ k , N→W) eqPf Γpf

split⟦Γ⟧ : ∀{k : model}{N→W : Node 
  → W k}{Γ1 Γ2 : Context} 
  → Γ⟦ Γ1 ++ Γ2 ⟧ k , N→W 
  → Γ⟦ Γ1 ⟧ k , N→W  × Γ⟦ Γ2 ⟧ k , N→W 
split⟦Γ⟧ {k}{N→W}{[]}{Γ2} ΓPF = (Data.Unit.tt , ΓPF)
split⟦Γ⟧ {k}{N→W}{(p+ , A , n) ∷ Γ1}{Γ2} ΓPF = 
  (((proj1 ΓPF , proj1 (split⟦Γ⟧ {k}{N→W}{Γ1}{Γ2} (proj2 ΓPF)))) , 
   proj2 (split⟦Γ⟧ {k}{N→W}{Γ1}{Γ2} (proj2 ΓPF)))
